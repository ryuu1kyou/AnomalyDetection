...
import { ThresholdRecommendationResultDto, ThresholdRecommendationRequestDto } from '../../models/anomaly-analysis.models';
...
export class ThresholdRecommendationsComponent implements OnInit {
  ...
  // Table columns
  displayedColumns = ['parameterName', 'currentValue', 'recommendedValue', 'priority', 'confidenceLevel', 'reason'];
  ...
  onGenerateRecommendations(): void {
    if (this.recommendationForm.valid) {
      ...
      this.anomalyAnalysisService.getThresholdRecommendations(request).subscribe({
        next: (result) => {
  ...
  getMetricValue(metrics: any, metricName: string): number {
    switch (metricName) {
      case 'Detection Rate': return metrics.detectionRate;
      case 'False Positive Rate': return metrics.falsePositiveRate;
      case 'False Negative Rate': return metrics.falseNegativeRate;
      case 'Precision': return metrics.precision;
      case 'Recall': return metrics.recall;
      case 'F1 Score': return metrics.f1Score;
      case 'Avg Detection Time': return metrics.averageDetectionTimeMs;
      default: return 0;
    }
  }
