...
**ユーザーストーリー:**
検出エンジニアとして、過去の異常検出事例やベストプラクティスをナレッジ記事として蓄積・共有し、チーム全体の知見を向上させたい。

**受け入れ基準:**
1. ユーザーがナレッジ記事を作成すると、システムはタイトル、内容、カテゴリ、タグを要求すること
2. 記事が関連するCAN信号や検出ロジックにリンクされると、システムは関連IDを保存すること
3. 記事が公開されると、システムはステータスを「公開済み」に変更して他のユーザーが閲覧可能にすること
4. ユーザーが記事を検索すると、システムはキーワードとタグでフィルタリングして関連度の高い記事を返すこと
5. 推奨機能が実行されると、システムは現在のコンテキスト（信号、ロジック、異常タイプ）に基づいて関連記事を自動提案すること
6. 記事が閲覧されると、システムは閲覧数を記録すること
7. ユーザーが記事を評価すると、システムは評価スコアを更新すること

### Req 18: 検出ロジックテンプレート機能

**ユーザーストーリー:**
検出エンジニアとして、よく使用する検出パターンをテンプレート化し、新規ロジック作成の効率を向上させたい。

**受け入れ基準:**
1. テンプレートが作成されると、システムは名前、説明、テンプレートタイプ、デフォルトパラメータを要求すること
2. テンプレートに必須信号が定義されると、システムは必須信号リストを保存すること
3. テンプレートから検出ロジックをインスタンス化すると、システムはデフォルトパラメータをコピーして新規ロジックを作成すること
4. テンプレートのパラメータが更新されると、システムは既存ロジックに影響を与えずにテンプレート自体のみを更新すること
5. テンプレートが使用されると、システムは使用回数をカウントすること
6. テンプレートがアクティブ/非アクティブ化されると、システムは可用性ステータスを変更すること

### Req 19: 安全性トレーサビリティ機能

**ユーザーストーリー:**
安全エンジニアとして、検出ロジックと安全要件の紐付けを管理し、ISO 26262等の安全規格への準拠を証明したい。

**受け入れ基準:**
1. トレース記録が作成されると、システムはエンティティID、エンティティタイプ、安全レベル、ASIL レベルを記録すること
2. 安全要件がリンクされると、システムは要件IDと要件タイプを保存すること
3. 検証が実行されると、システムは検証ステータスを「検証済み」に更新して検証日時を記録すること
4. 監査レポートが生成されると、システムはすべてのトレース記録と検証状況をまとめたレポートを出力すること
5. 安全レベルが変更されると、システムは変更履歴を記録して監査証跡を残すこと
6. 未検証のトレース記録が検索されると、システムは検証待ちの項目をフィルタリングして返すこと

### Req 20: 互換性分析機能

**ユーザーストーリー:**
システム管理者として、CAN信号や検出ロジックのバージョン間の互換性を分析し、アップグレード時の影響を事前に把握したい。

**受け入れ基準:**
1. 互換性分析が要求されると、システムは2つのバージョンを比較して差分を抽出すること
2. バージョン比較が実行されると、システムは信号仕様、パラメータ、マッピングの変更点を特定すること
3. 破壊的変更が検出されると、システムは変更の種類（削除、型変更、必須パラメータ追加等）を分類して警告すること
4. 互換性レポートが生成されると、システムは影響範囲、推奨アクション、リスクレベルを含むレポートを出力すること
5. マイグレーション計画が作成されると、システムは必要な変更手順を提示すること

### Req 21: 閾値最適化機能

**ユーザーストーリー:**
検出エンジニアとして、機械学習や統計分析を用いて検出ロジックの閾値を最適化し、誤検知と未検知のバランスを改善したい。

**受け入れ基準:**
1. 最適化が要求されると、システムは過去の検出結果を分析して最適な閾値候補を計算すること
2. 最適化アルゴリズムが実行されると、システムは適合率、再現率、F1スコアを最大化する閾値を提案すること
3. 最適化履歴が記録されると、システムは実行日時、使用アルゴリズム、結果パラメータを保存すること
4. 最適化された閾値が適用されると、システムは検出ロジックのパラメータを更新して変更履歴を記録すること
5. 最適化結果が比較されると、システムは適用前後のメトリクスを可視化して改善効果を示すこと
6. ロールバックが実行されると、システムは以前の閾値設定に戻すことができること
